...
The natural parameter is $\theta=\mu$, and hence

$$
b'(\theta)=\theta,\qquad b''(\theta)=1.
$$

Property 1 therefore gives
...
Since $\theta=\log\lambda$, we have $e^\theta=\lambda$ and
...
Nonnegativity follows from $f\ge0$ and $\sigma>0$. Let
$z=(x-\mu)/\sigma$, so $x=\mu+\sigma z$ and $dx=\sigma dz$. Then
...
Thus $g$ is nonnegative and integrates to one.
...
A location transformation replaces $x$ in the standard density by $x-\mu$.
The original support condition $x\ge0$ becomes $x-\mu\ge0$, or $x\ge\mu$.
The positive part of the density is therefore shifted by $\mu$ without changing
its shape. Its normalization is
...
Here $\mu$ is also the lower endpoint of the support and may be interpreted as a
threshold parameter in lifetime or reaction-time models.
...
For the “if” direction, let $Z$ have density $f$ and set
$X=\sigma Z+\mu$. Since $\sigma>0$, the inverse transformation is
$z=(x-\mu)/\sigma$, whose derivative has absolute value $1/\sigma$.
The one-dimensional change-of-variable formula gives

$$
f_X(x)=f\left(\frac{x-\mu}{\sigma}\right)\frac1\sigma.
$$

Conversely, suppose $X$ has this density and define
$Z=(X-\mu)/\sigma$, so $X=\sigma Z+\mu$. Applying the transformation
formula in the other direction gives
...
Thus $Z$ has the standard density $f$, proving both directions.

</details>

::: {.source-theorem}
**Theorem 3.5.7 (moments under a location--scale transformation)** If $Z$
has density $f$ and $E(Z)$ and $\operatorname{Var}(Z)$ exist, then for
$X=\sigma Z+\mu$,
...
In particular, if $E(Z)=0$ and $\operatorname{Var}(Z)=1$, then
$E(X)=\mu$ and $\operatorname{Var}(X)=\sigma^2$.
:::

<details class="course-details proof-details">
<summary><strong>Expand proof</strong></summary>

By Theorem 3.5.6, use the representation $X=\sigma Z+\mu$. Linearity of
expectation yields
...
A constant shift does not change variance, while multiplication by a constant
multiplies variance by its square. Hence
