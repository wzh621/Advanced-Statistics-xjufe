# Interval Estimation {#interval-estimation}

This chapter corresponds to *chap-9.tex*, its three LaTeX fragments, and `HPD_case.R`. It covers confidence intervals, test inversion, pivots, Bayesian credible sets, interval length and coverage, and highest-posterior-density regions. All source figures are retained; the HPD code is made reproducible without an additional MCMC package.

## Interval estimators, coverage, and confidence coefficient {#interval-estimator-definitions}

::: {.definition}
An **interval estimator** of a real parameter $\theta$ is a pair of sample functions $L(\mathbf X),U(\mathbf X)$ with $L(\mathbf x)\le U(\mathbf x)$. After observing $\mathbf x$, report $[L(\mathbf x),U(\mathbf x)]$.
:::

::: {.definition}
The **coverage probability** is
$$P_\theta\{L(\mathbf X)\le\theta\le U(\mathbf X)\},$$
usually a function of $\theta$. The **confidence coefficient** is its infimum over $\Theta$.
:::

## Inverting hypothesis tests {#inverting-tests}

::: {.theorem .source-numbered}
**Theorem 9.2.2 (Test--confidence-set duality).** For each $\theta_0$, let $A(\theta_0)$ be the acceptance region of a level $\alpha$ test of $H_0:\theta=\theta_0$. Then
$$\mathcal C(\mathbf x)=\{\theta_0:\mathbf x\in A(\theta_0)\}$$
is a confidence set with coverage at least $1-\alpha$. Conversely, a confidence set defines level $\alpha$ acceptance regions by $A(\theta_0)=\{\mathbf x:\theta_0\in\mathcal C(\mathbf x)\}$.
:::

<details class="course-details proof-details">
<summary><strong>Expand proof</strong></summary>

Since $A(\theta_0)$ accepts a level-$\alpha$ test,
$P_{\theta_0}\{\mathbf X\in A(\theta_0)\}\ge1-\alpha$. Because
$\mathcal C(\mathbf X)=\{\vartheta:\mathbf X\in A(\vartheta)\}$,

$$P_\theta\{\theta\in\mathcal C(\mathbf X)\}
=P_\theta\{\mathbf X\in A(\theta)\}\ge1-\alpha.$$

Conversely, if $\mathcal C(\mathbf X)$ has coverage at least $1-\alpha$, define
$A(\theta_0)=\{\mathbf x:\theta_0\in\mathcal C(\mathbf x)\}$. Its Type I error probability is

$$P_{\theta_0}\{\mathbf X\notin A(\theta_0)\}
=P_{\theta_0}\{\theta_0\notin\mathcal C(\mathbf X)\}\le\alpha,$$

so it defines a level-$\alpha$ test.

</details>

::: {.example .source-numbered}
**Example 9.2.1 (Inverting a normal test).** If $X_i\sim N(\mu,\sigma^2)$ with known $\sigma$, invert the two-sided level-$\alpha$ test to obtain a confidence interval for $\mu$.
:::

<details class="course-details solution-details">
<summary><strong>View detailed solution</strong></summary>

The test accepts $H_0:\mu=\mu_0$ when
$$|\bar X-\mu_0|\le z_{\alpha/2}\frac{\sigma}{\sqrt n}.$$
Inverting in $\mu_0$ gives
$$\left[\bar X-z_{\alpha/2}\frac{\sigma}{\sqrt n},
\bar X+z_{\alpha/2}\frac{\sigma}{\sqrt n}\right].$$

</details>

<div class="figure" style="text-align: center">
<img src="../images/ch09/501011.png" alt="Source diagram relating acceptance regions and confidence intervals" width="58%" />
<p class="caption">(\#fig:en-chap09-source-test-inversion)Source diagram relating acceptance regions and confidence intervals</p>
</div>

## Inverting an exponential-mean LRT {#inverting-exponential-lrt}

::: {.example .source-numbered}
**Example 9.2.3 (Inverting an exponential LRT).** For exponential observations with mean (scale) $\lambda$, invert the LRT of $H_0:\lambda=\lambda_0$ to obtain a $1-\alpha$ interval.
:::

<details class="course-details solution-details">
<summary><strong>View detailed solution</strong></summary>

For exponential observations with mean (scale) $\lambda$, the LR statistic for $H_0:\lambda=\lambda_0$ is
$$
\lambda_{\mathrm{LR}}(\mathbf x)=
\frac{\lambda_0^{-n}e^{-\sum x_i/\lambda_0}}{(\sum x_i/n)^{-n}e^{-n}}
=\left(\frac{\sum x_i}{n\lambda_0}\right)^ne^ne^{-\sum x_i/\lambda_0}.
$$
Inversion gives an interval $[L(T),U(T)]$, $T=\sum x_i$, whose endpoints satisfy
$$\left(\frac{T}{L(T)}\right)^ne^{-T/L(T)}=
\left(\frac{T}{U(T)}\right)^ne^{-T/U(T)}.$$
Writing $T/L=a$, $T/U=b$, $a>b$, gives $a^ne^{-a}=b^ne^{-b}$. Since $T/\lambda\sim\operatorname{Gamma}(n,1)$, choose $a,b$ so that $P\{b\le T/\lambda\le a\}=1-\alpha$.

The inverted interval is therefore $[T/a,T/b]$; coverage and equal-likelihood conditions jointly determine the endpoints.

</details>

<div class="figure" style="text-align: center">
<img src="../images/ch09/501012.png" alt="Source diagram for inversion of the exponential-model LRT" width="72%" />
<p class="caption">(\#fig:en-chap09-source-lrt-inversion)Source diagram for inversion of the exponential-model LRT</p>
</div>

## Pivotal quantities and location--scale examples {#pivotal-quantities}

::: {.definition}
A **pivotal quantity** $Q(\mathbf X,\theta)$ has a sampling distribution free of all unknown parameters. If
$P\{a\le Q(\mathbf X,\theta)\le b\}=1-\alpha$, inversion gives
$\mathcal C(\mathbf x)=\{\theta:a\le Q(\mathbf x,\theta)\le b\}$.
:::

For a location family, $\bar X-\mu$ is pivotal; for a scale family, $\bar X/\sigma$ is pivotal; and for a location--scale family, $(\bar X-\mu)/\sigma$ is distribution-free, although with two unknown parameters one must specify the target and nuisance treatment.

::: {.example}
If exponential observations have scale $\lambda$, then $T=\sum_iX_i\sim\operatorname{Gamma}(n,\lambda)$ and
$$\frac{2T}{\lambda}\sim\chi^2_{2n}.$$
With $a=\chi^2_{2n,\alpha/2}$ and $b=\chi^2_{2n,1-\alpha/2}$,
$$P_\lambda\left\{\frac{2T}{b}\le\lambda\le\frac{2T}{a}\right\}=1-\alpha.$$
The source's specific 95% example uses 9.59 and 34.17, giving $[2T/34.17,2T/9.59]$.
:::

## Bayesian credible sets {#bayesian-credible-sets}

Frequentist probability comes from repeated sampling, so a random interval covers a fixed parameter. In Bayesian analysis $\theta$ is random under the posterior, and
$$P(\theta\in A\mid\mathbf x)=\int_A\pi(\theta\mid\mathbf x)\,d\theta$$
is the posterior probability that the parameter lies in credible set $A$.

::: {.example}
If $X_i\sim\operatorname{Poisson}(\lambda)$ and the shape--scale prior is $\lambda\sim\operatorname{Gamma}(a,b)$, then
$$\lambda\mid\textstyle\sum X_i=\sum x_i
\sim\operatorname{Gamma}\left(a+\sum x_i,[n+1/b]^{-1}\right).$$
An equal-tail interval uses posterior quantiles $\alpha/2$ and $1-\alpha/2$. For $a=b=1,n=10,\sum x_i=6$, the source gives the 90% interval $[0.299,1.077]$.
:::

::: {.example}
For $X_i\sim N(\theta,\sigma^2)$ and $\theta\sim N(\mu,\tau^2)$,
$$\delta^B(\bar x)=\frac{\sigma^2\mu+n\tau^2\bar x}{\sigma^2+n\tau^2},\qquad
\operatorname{Var}(\theta\mid\bar x)=\frac{\sigma^2\tau^2}{\sigma^2+n\tau^2}.$$
The $1-\alpha$ credible interval is
$\delta^B(\bar x)\pm z_{\alpha/2}\sqrt{\operatorname{Var}(\theta\mid\bar x)}$.
:::

## Credible probability versus frequentist coverage {#credible-versus-coverage}

Let $\gamma=\sigma^2/(n\tau^2)$. The frequentist coverage of the normal Bayesian interval is
$$P\left\{-\sqrt{1+\gamma}z_{\alpha/2}+
\frac{\gamma(\theta-\mu)}{\sigma/\sqrt n}\le Z\le
\sqrt{1+\gamma}z_{\alpha/2}+
\frac{\gamma(\theta-\mu)}{\sigma/\sqrt n}\right\},$$
$Z\sim N(0,1)$. It generally depends on $\theta$, so a $1-\alpha$ credible interval is not automatically a $1-\alpha$ confidence interval.

Conversely, the posterior probability of the usual confidence interval $|\theta-\bar x|\le z_{\alpha/2}\sigma/\sqrt n$ equals
$$P\left\{-\sqrt{1+\gamma}z_{\alpha/2}+
\frac{\gamma(\bar x-\mu)}{\sqrt{1+\gamma}\sigma/\sqrt n}\le Z\le
\sqrt{1+\gamma}z_{\alpha/2}+
\frac{\gamma(\bar x-\mu)}{\sqrt{1+\gamma}\sigma/\sqrt n}\right\},$$
which depends on the prior location.

## Evaluating interval length and coverage {#interval-size-coverage}

Good intervals combine high coverage with small size. Coverage may be summarized by the confidence coefficient or average coverage; size is length in one dimension and often volume in higher dimensions.

::: {.theorem .source-numbered}
**Theorem 9.3.2 (Shortest probability interval for a unimodal density).** For a unimodal density $f$, if $[a,b]$ contains a mode and satisfies
$\int_a^bf(x)dx=1-\alpha$ and $f(a)=f(b)>0$, then it is shortest among intervals with probability $1-\alpha$.
:::

<details class="course-details proof-details">
<summary><strong>Expand proof</strong></summary>

Let $[a',b']$ satisfy $b'-a'<b-a$. It suffices to show that its probability is below $1-\alpha$. Take $a'\le a$; the opposite placement is symmetric.

If $b'\le a$, unimodality and $a\le x^*\le b$ imply

$$\int_{a'}^{b'}f
\le f(a)(b'-a')<f(a)(b-a)\le\int_a^bf=1-\alpha.$$

If $b'>a$, then $a'\le a<b'<b$, and

$$\int_{a'}^{b'}f-\int_a^bf
=\int_{a'}^af-\int_{b'}^bf
\le f(a)(a-a')-f(b)(b-b')<0,$$

where $f(a)=f(b)>0$ and the last inequality is equivalent to $b'-a'<b-a$. Thus every shorter interval contains less probability, proving minimality.

</details>

For a standard normal pivot, the source compares 90% intervals $(-1.34,2.33)$, $(-1.44,1.96)$, and $(-1.65,1.65)$, with lengths 3.67, 3.40, and 3.30. Equal tails are optimal. For the unknown-variance normal $t$ interval, length is $(b-a)S/\sqrt n$ and expected length is $(b-a)c(n)\sigma/\sqrt n$, so symmetric endpoints remain optimal.

## Shortest pivotal intervals {#shortest-pivotal-interval}

::: {.example .source-numbered}
**Example 9.3.4 (Shortest pivotal interval for a gamma scale).** Let $X\sim\operatorname{Gamma}(k,\beta)$. Explain why equal-density endpoints for the pivot do not automatically minimize the interval length for $\beta$.
:::

<details class="course-details solution-details">
<summary><strong>View detailed solution</strong></summary>

If $X\sim\operatorname{Gamma}(k,\beta)$, then $Y=X/\beta\sim\operatorname{Gamma}(k,1)$. From $P(a\le Y\le b)=1-\alpha$,
$$\frac{x}{b}\le\beta\le\frac{x}{a}.$$
Its length is proportional to $1/a-1/b$, not $b-a$, so $f_Y(a)=f_Y(b)$ is not directly optimal. With $b=b(a)$, solve
$$\min_a\left\{\frac1a-\frac1{b(a)}\right\},\qquad
\int_a^{b(a)}f_Y(y)dy=1-\alpha.$$

The target is the interval length after transformation back to parameter space, which is why Theorem 9.3.2 cannot be applied mechanically to the pivot scale.

</details>

## Highest-posterior-density regions {#highest-posterior-density}

::: {.definition}
An HPD region minimizes volume among sets of posterior probability at least $1-\alpha$. For a unimodal posterior, it is
$$\{\theta:\pi(\theta\mid\mathbf x)\ge k\},\qquad
\int_{\{\theta:\pi(\theta\mid\mathbf x)\ge k\}}
\pi(\theta\mid\mathbf x)d\theta=1-\alpha,$$
so its endpoints have equal posterior density.
:::

For the Poisson--Gamma example with $a=b=1,n=10,\sum x_i=6$, the source gives the 90% HPD interval $[0.253,1.005]$.


``` r
observed_data <- c(3, 2, 4, 1, 5)
prior_shape <- 2
prior_rate <- 1
post_shape <- prior_shape + sum(observed_data)
post_rate <- prior_rate + length(observed_data)
cred_mass <- 0.95
set.seed(123)
posterior_samples <- sort(rgamma(100000, shape = post_shape, rate = post_rate))
window_size <- floor(cred_mass * length(posterior_samples))
widths <- posterior_samples[(window_size + 1):length(posterior_samples)] -
  posterior_samples[1:(length(posterior_samples) - window_size)]
lower_index <- which.min(widths)
hpd <- posterior_samples[c(lower_index, lower_index + window_size)]
x_grid <- seq(0, qgamma(0.999, post_shape, rate = post_rate), length.out = 500)
plot(x_grid, dgamma(x_grid, post_shape, rate = post_rate), type = "l", lwd = 2,
     xlab = expression(lambda), ylab = "Posterior density")
abline(v = hpd, col = "#C43C39", lty = 2, lwd = 2)
abline(v = mean(posterior_samples), col = "#1F77B4", lwd = 2)
legend("topright", c(sprintf("95%% HPD [%.2f, %.2f]", hpd[1], hpd[2]), "Posterior mean"),
       col = c("#C43C39", "#1F77B4"), lty = c(2, 1), lwd = 2, bty = "n")
```

<div class="figure" style="text-align: center">
<img src="09-interval-estimation_files/figure-html/en-chap09-hpd-gamma-1.png" alt="A 95% gamma-posterior HPD interval using the source data, prior, and posterior sampling" width="90%" />
<p class="caption">(\#fig:en-chap09-hpd-gamma)A 95% gamma-posterior HPD interval using the source data, prior, and posterior sampling</p>
</div>

<div class="figure" style="text-align: center">
<img src="../images/ch09/HPD.png" alt="Source comparison of three interval estimators" width="50%" />
<p class="caption">(\#fig:en-chap09-source-hpd)Source comparison of three interval estimators</p>
</div>

## Confidence intervals under repeated sampling {#confidence-interval-repeated-sampling}

A confidence coefficient belongs to the interval-producing procedure, not to one fixed interval after observation. The experiment below fixes $\mu=0,\sigma=1$, draws 80 normal samples of size 20, and constructs a 95% $z$ interval from each sample.


``` r
set.seed(9409)
mu_true <- 0
sigma_true <- 1
n_ci <- 20
m_ci <- 80
sample_means <- replicate(m_ci, mean(rnorm(n_ci, mu_true, sigma_true)))
margin <- qnorm(0.975) * sigma_true / sqrt(n_ci)
lower_ci <- sample_means - margin
upper_ci <- sample_means + margin
covered <- lower_ci <= mu_true & mu_true <= upper_ci
interval_color <- ifelse(covered, "#2C7FB8", "#D7301F")

plot(NA, xlim = range(c(lower_ci, upper_ci)), ylim = c(1, m_ci),
     xlab = expression(mu), ylab = "Repeated-sample index")
abline(v = mu_true, lwd = 2, lty = 2, col = "#263238")
arrows(lower_ci, seq_len(m_ci), upper_ci, seq_len(m_ci),
       angle = 90, code = 3, length = 0.025, col = interval_color)
points(sample_means, seq_len(m_ci), pch = 16, cex = 0.45, col = interval_color)
legend("topright", c("Covers the true mean", "Misses the true mean"),
       col = c("#2C7FB8", "#D7301F"), lwd = 2, bty = "o", bg = "white")
```

<div class="figure" style="text-align: center">
<img src="09-interval-estimation_files/figure-html/en-chap09-repeated-confidence-intervals-1.png" alt="Eighty 95% confidence intervals; red intervals fail to cover the true mean" width="90%" />
<p class="caption">(\#fig:en-chap09-repeated-confidence-intervals)Eighty 95% confidence intervals; red intervals fail to cover the true mean</p>
</div>

In this run, 75 intervals cover $\mu=0$. A rerun changes which particular intervals miss; only the long-run coverage proportion approaches $0.95$. After one interval is observed, frequentist inference does not make the fixed parameter random.

## Comparing intervals for a binomial proportion {#binomial-interval-comparison}

For $Y\sim\operatorname{Bin}(n,p)$, familiar intervals can behave differently with small samples or when $p$ is near 0 or 1. We compare Wald, Wilson, likelihood-ratio inversion, and the exact Clopper--Pearson interval. At each true $p$, coverage is summed over the exact probabilities of $Y=0,\ldots,n$, so the curves contain no Monte Carlo error.


``` r
source("../R/inference_labs.R")
p_grid <- seq(0.01, 0.99, by = 0.01)
interval_performance <- binomial_interval_coverage(20, p_grid, conf.level = 0.95)
method_colors <- c("#D7301F", "#1F78B4", "#33A02C", "#6A3D9A")

old_par <- par(mfrow = c(1, 2), mar = c(4, 4, 2.5, 1))
matplot(p_grid, interval_performance$coverage, type = "l", lty = 1,
        lwd = 2, col = method_colors, ylim = c(0.55, 1),
        xlab = expression(p), ylab = "Actual coverage")
abline(h = 0.95, lty = 2, lwd = 1.5)
legend("bottom", colnames(interval_performance$coverage),
       col = method_colors, lty = 1, lwd = 2, cex = 0.72, bty = "n")
matplot(p_grid, interval_performance$mean_length, type = "l", lty = 1,
        lwd = 2, col = method_colors,
        xlab = expression(p), ylab = "Mean interval length")
```

<div class="figure" style="text-align: center">
<img src="09-interval-estimation_files/figure-html/en-chap09-binomial-interval-coverage-1.png" alt="Exact coverage and mean length of four binomial-proportion intervals for n=20" width="90%" />
<p class="caption">(\#fig:en-chap09-binomial-interval-coverage)Exact coverage and mean length of four binomial-proportion intervals for n=20</p>
</div>

``` r
par(old_par)
```

The Wald interval severely undercovers near the boundaries, whereas Wilson coverage is usually closer to the nominal level. Clopper--Pearson controls tail probabilities but is often conservative and longer. The likelihood-ratio interval directly inverts the likelihood-ratio test from Chapter 8. A method should therefore be judged by both coverage and length, not by algebraic simplicity alone.

## Running case: interval estimation of the defect rate {#defect-rate-intervals}

Continue with the product-inspection data $n=200,y=16$. The table reports four two-sided 95% intervals. The calculation is stored in `R/inference_labs.R` for reuse in later cases.


``` r
source("../R/inference_labs.R")
defect_intervals <- binomial_intervals(y = 16, n = 200, conf.level = 0.95)
knitr::kable(round(defect_intervals, 4),
             col.names = c("Lower", "Upper"),
             caption = "Four two-sided 95% intervals for the product defect rate")
```



Table: (\#tab:en-chap09-defect-rate-intervals)Four two-sided 95% intervals for the product defect rate

|                 |  Lower|  Upper|
|:----------------|------:|------:|
|Wald             | 0.0424| 0.1176|
|Wilson           | 0.0498| 0.1260|
|Likelihood ratio | 0.0477| 0.1229|
|Clopper-Pearson  | 0.0464| 0.1267|

All four intervals express uncertainty around the point estimate $0.08$, but the narrower lower end of the Wald interval should not be mistaken for greater reliability. There is also an important test--interval detail: Chapter 8 used a **one-sided** test at $5\%$, whereas this table reports **two-sided** 95% intervals, so they are not direct inversions of one another. The one-sided 5% test corresponds to a 95% one-sided lower bound (or the matching two-sided 90% equal-tail interval). The exact two-sided 90% lower endpoint is 0.0508, which exceeds the benchmark $0.05$ and agrees with the one-sided test.

## Chapter summary {#interval-estimation-summary}

Confidence intervals arise by test inversion or pivots and make repeated-sampling statements; credible intervals concern the posterior after observing data. Evaluation must balance coverage and size, and shortest-interval conditions must be derived for the final parameter-scale length.
